...
```plaintext
┌─────────────────┐     ┌─────────────────────────────────────────┐
│   Client        │     │           Daemon (HTTP Service)         │
│                 │     │  ┌─────────┐  ┌─────────┐  ┌─────────┐  │
│  POST /sync ────┼────►│  │handlers │  │  tasks  │  │  state  │  │
│  GET /tasks/id  │◄────│  └────┬────┘  └────┬────┘  └────┬────┘  │
│  GET /metrics   │◄────│       │            │            │        │
└─────────────────┘     │  ┌────┴────────────┴────────────┘        │
                        │  │            server                     │
...
- **core**：纯逻辑库，负责 ModelScope API 调用、SHA256 校验、HTTP 流式下载和同步编排
- **daemon**：薄控制层，提供 HTTP API、管理异步任务生命周期、暴露可观测性接口
